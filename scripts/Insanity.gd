...
func _on_ending_updated(text: String, texture: String = ""):
	...
	if scene_label and scene_label is Label:
		scene_label.text = "Insanity"
	if(texture != ""):
		$Padded.texture = load(texture)
	GameManager.display_dialog(GameManager.endings[text])
	await get_tree().process_frame
	AudioManager.play_music(load("res://assets/audio/67_Asylum.ogg"))
